...
class Solution{
  public:
    ...
    // Getting Smallest Values in Left aand Right Direction //
    vector<int> nextSmallestElement(int arr[], int n, char direction){
        // Your code here
        int len = 0,i;
    ...
    //Function to find largest rectangular area possible in a given row/histogram.
    ...
    int maxArea(int mat[MAX][MAX], int n, int m) {
        ...
        int maxAns = INT_MIN;
        maxAns = max(maxAns, getMaxArea(mat[0], m));
        
        for(int i=1;i<n;i++){
            for(int j=0;j<m;j++)
                if( mat[i][j] )
                    mat[i][j] += mat[i-1][j]; 
            maxAns = max(maxAns, getMaxArea(mat[i], m));
        }
